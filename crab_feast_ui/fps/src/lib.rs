...
struct FpsData {
    ...
    frame_timestamps: [Option<f64>; 100],
    current_index: usize,
    initialized: bool,
}
...
impl Plugin for FPSPlugin {
    fn build(&self, app: &mut App) {
        app.insert_resource(FpsData::default()).add_systems(
            Update,
            (fps_setup.run_if(not(resource_exists::<FpsUI>)), update_fps),
        );
    }
}
...
fn fps_setup(mut commands: Commands) {
    ...
    commands.insert_resource(FpsUI);
}
